...
int Group_FaceComparison( int candidate_index[],int total_trainface,int train_feature_count[], int probe_feature_count,double Probe_feature_Vectors[][41][2])
{
	int i,j,k,m;
	int MaxSimilarity_count[Group_Train]={0};
	double OveralSimilarity[Group_Train]={0.0};
	double temp_max_similarity=0.0;
	double point_peak_similarity=0.0;
	int max_similarity_index=0;
	int point_peak_index;
	int point_peak_existed=0;
	double similarity=0.0;
	double similarity_matrix[500][Group_Train]={0.0};
	double temp_MaxSimilarity_count=0.0,temp_feature_count=0.0;
	...
	for (i=0;i<probe_feature_count;i++)
	{
		...
		for (j=0;j<total_trainface;j++)
		{
			...
			temp_index=candidate_index[j];
			for (k=0;k<train_feature_count[temp_index];k++)
			{
				similarity=0.0;
				train_xcoordiante=Feature_Vectors[temp_index][k][0][0];
				train_ycoordiante=Feature_Vectors[temp_index][k][0][1];
				distance=sqrt((train_xcoordiante-probe_xcoordiante)*(train_xcoordiante-probe_xcoordiante)+(train_ycoordiante-probe_ycoordiante)*(train_ycoordiante-probe_ycoordiante));
				if (distance<=thresh_distance)
				{
					dotproduct=0.0;
					...
					for (m=1;m<41;m++)  //the first one is position information
					{
						dotproduct=dotproduct+sqrt(complex_modulus(Feature_Vectors[temp_index][k][m]))*sqrt(complex_modulus(Probe_feature_Vectors[i][m]));
						train_feature_sqr = train_feature_sqr + complex_modulus(Feature_Vectors[temp_index][k][m]);
						probe_feature_sqr = probe_feature_sqr + complex_modulus(Probe_feature_Vectors[i][m]);
					}
					similarity=dotproduct/sqrt(train_feature_sqr*probe_feature_sqr);
					if (similarity>last_similarity)
