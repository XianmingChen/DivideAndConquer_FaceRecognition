...
void main()
{
	...
	int candidate[Total_train_face]={0};
	int new_candidate[Total_train_face]={0};
	int candidate_count;
	int active_train;
	int round_index,temp_index;
	int round, remainder;
	...
	total_start=clock();
	start=clock();
	printf("%d images of Binary EBGM begins...\n",Total_probe_face);
	...
	for (i=0;i<Total_probe_face;i++)
	{
		...
		each_feature_count=0;
				...
				active_train=Total_train_face;
		for (j=0;j<active_train;j++)
		{
			candidate[j]=j;
		}

		//tournament rounds
		while(active_train>Group_Train)
		{
			candidate_count=0;
			round_index=0;

			round=active_train/Group_Train;
			remainder=active_train%Group_Train;
			for(j=0;j<round;j++)
			{
				temp_index=Group_FaceComparison(&candidate[j*Group_Train],Group_Train,train_feature_count,probe_feature_count,Each_Feature_Vectors)+round_index;
				new_candidate[candidate_count]=candidate[temp_index];
				candidate_count++;
				round_index=round_index+Group_Train;
			}
			if (remainder>0)
			{
				if (remainder==1)
					temp_index=round_index; //no need to compare if only one candidate
				else
					temp_index=Group_FaceComparison(&candidate[round*Group_Train],remainder,train_feature_count,probe_feature_count,Each_Feature_Vectors)+round_index;
				
				new_candidate[candidate_count]=candidate[temp_index];
				candidate_count++;
			}

			memcpy(candidate,new_candidate,sizeof(int)*candidate_count);
			if (remainder==0)
				active_train=active_train/Group_Train;
			else
				active_train=active_train/Group_Train+1;
		}
		//final matching round begins
		temp_index=Group_FaceComparison(candidate,candidate_count,train_feature_count,probe_feature_count,Each_Feature_Vectors);
		Accuracy=candidate[temp_index];
